    ...
    .then((savedUser)=>{
        ...
        bcrypt.hash(password,12).then((hashedPassword)=>{
            const user=new Student({
                name,email,roll_no,branch,admission_year:year,phone_no,password:hashedPassword,isAdmin
            });
            user.save().
            then((user)=>{
...
router.post("/signin",(req,res)=>{
    ...
    if(!roll_no||!password)
    {
        return res.status(422).json({error: "please add roll_no and passowrd"});
    }
    Student.findOne({roll_no: roll_no})
    .then((savedUser)=>{
        if(!savedUser){
            res.status(422).json({ error:"Invalid roll_no or passowrd" });
        }
        bcrypt
...
router.get("/allStudents",(req,res)=>{
    Student.find().sort({createdAt: -1}).then(data=>res.status(200).json(data))
});
